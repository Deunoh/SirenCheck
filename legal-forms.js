// Nomenclature officielle INSEE des catégories juridiques (variable "nature_juridique"
// de la base SIRENE). L'API recherche-entreprises ne renvoie que le code brut, jamais
// son libellé : ces tables servent à retrouver un intitulé lisible sans jamais afficher
// de code opaque à l'utilisateur.
//
// Sources :
// - https://xml.insee.fr/schema/cj-enum.html (énumération complète des codes à 4 chiffres)
// - https://www.insee.fr/fr/information/2028129 (code 1000 "Entrepreneur individuel",
//   introduit en juillet 2018, absent du schéma XML ci-dessus)
const LEGAL_FORM_LABELS = {
  ...
  "5430": "Safer en SARL",
  "5431": "SARL mixte d'intérêt agricole (SIMA)",
  ...
  "7450": "Établissement public, cercle et foyer dans les armées",
...
// Repli par grande catégorie (1er chiffre du code). Cette structure à 9 entrées est
// stable depuis la création de la nomenclature SIRENE : elle garantit un libellé
// compréhensible même pour un code à 4 chiffres pas encore répertorié ci-dessus,
// plutôt que d'afficher un code brut.
const LEGAL_FORM_CATEGORY_LABELS = {
  1: "Personne physique",
...
function getLegalFormLabel(code) {
  if (!code) return null;
  return LEGAL_FORM_LABELS[code] || LEGAL_FORM_CATEGORY_LABELS[code[0]] || null;
}
